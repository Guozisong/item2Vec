...
def test_main_reports_only_valid_baskets_and_reuses_precomputed_indexes(monkeypatch, tmp_path, capsys):
    captured = {}
    ...
    monkeypatch.setitem(sys.modules, "pandas", types.SimpleNamespace(read_csv=lambda _path: FakeDataframe()))
    ...
    monkeypatch.setattr(
        ...
        "train_item2vec_with_bert_init",
        lambda embedding, item2index, baskets, **kwargs: (
            captured.update({"baskets": baskets, **kwargs}) or (embedding, object())
        ),
    )
    monkeypatch.setattr(training, "write_trained_embedding", lambda _embedding, _path: tmp_path / "trained_item.featCLS")

    training.main([str(tmp_path / "raw"), str(tmp_path / "downstream")])

    assert captured["basket_index"] == [["0", "1"]]
    assert "已构建 1 个有效购物篮，包含 2 个商品向量。" in capsys.readouterr().out
